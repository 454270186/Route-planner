...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    /*while(!(current_node->neighbors.empty()))
    {
      ...
      neighbor_node = *(current_node->neighbors.back());
      current_node->neighbors.pop_back();
...
{
  float f1 = a->h_value + (double)a->g_value;
  float f2 = b->h_value + (double)b->g_value;
  return f1 > f2;
}
...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    RouteModel::Node *sml_f_node = this->open_list.back();
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    std::vector<RouteModel::Node> rev_path;
    ...
    while(current_node->parent)
    {
      ...
      float c_p_distance = current.distance(parent);
      distance += c_p_distance;
      rev_path.push_back(current);
      current_node = &parent;
    }
  
    rev_path.push_back(*start_node); //疏漏的一行代码
  
    while(!rev_path.empty())
    {
      path_found.push_back(rev_path.back());
      rev_path.pop_back();
    }
